...
| limitation·F1 | P2-2「题干含金标考点关键词」口径为**注入链路一致性**（fake LLM 原样回显考点清单块），不是真实「题目与清单相关性」证据；实测随抽取结果（LLM 非确定性）波动：89% → 91% → 80% → 84% → 82%（最终审查轮两轮复跑 38/45 与 37/45，标准 ≥80%，贴近阈值） | 真实相关性待后续用真实 LLM 抽样判定；建议同时固定抽样语料与缓解抖动 |
