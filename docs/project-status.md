...
**阶段：P2 交付完成 + 端到端验收通过（2026-09-18 复跑确认）——简历解析入库（P2-1）/ 双来源出题（P2-2）/ SqliteSaver 跨重启持久化（P2-3）/ web_verify 事实核验（P2-4）/ 端到端链路（P2-5）全部落地。第 2 轮审查暴露的 app 侧缺陷（流式 skip 出题失败）**已修复（`8e2933a`）**；**最终审查轮（2026-09-18）修复 1 blocker + 2 major + 3 minor（`eb68045` 搜索 Key 惰性解析 / `204d81c` API Key 不再落盘 / 验收与文档口径修正）**后全量复跑实测：**门禁 passed=15 failed=0（退出码 0）、观察项 1 项 PASS、依赖缺失 dep=0**（门禁 14 → 15 = 新增「P2-4 Key 通路」项）；**充值后真实 LLM 全量复跑（2026-09-18）再次确认：门禁 passed=15 failed=0（退出码 0）、观察项 1 PASS、dep=0**——P2-1 解析成功率 20/20=100% 与字段命中率 20/20=100%、P2-2 注入链路一致性 40/45=89%（题干命中 3/3）、P2-4/P2-5 事实核验均 verified、retry 观察项 22.3s（本轮无代码改动，pytest 280 / ruff / build 证据维持）；P1-3（28%，标准 ≥90%）与 P1-4 decline 子项搁置项顺延，待真实检索环境恢复后复测（标准未调整）**
...
- [x] **P2 最终审查修复轮（2026-09-18）**：修复审查发现的 1 blocker + 2 major + 3 minor，**本轮改动了 `app/` 代码**（既有测试仅新增用例，未改既有断言语义）。① **BLOCKER**：`VerifyContext` 构造期固化 `BochaClient` 导致配置页设置的搜索 Key 永远不生效（`search()` 恒抛 `BochaError` → 恒 `unconfirmed` + `sources` 恒空）→ 改为**每次 `verify()` 惰性按当前 Key 解析**（KeyStore → `cfg.verify.api_key` 回退；Key 变化才重建、同 Key 复用），`cfg.verify.enabled=False` 真正短路跳过，lifespan 按 LLM Key 同模式用 `.env` 的 `VERIFY_API_KEY` 种子（`eb68045`）。② **MAJOR（安全）**：`_api_key` 随 SqliteSaver 明文落盘（本地库 checkpoint/writes 内含 `sk-` 明文）→ **方案 A 持久化边界脱敏**：`SanitizingSqliteSaver`（`put_writes` 丢弃 `_api_key` 通道写入 + `put` 剔除 `updated_channels` 通道名）+ `SanitizedSerde`（`dumps*` 递归剔键），并修正 `state.py` 的「不持久化」注释与 `Score` 缺失的 `topic`/`verification` 声明（`204d81c`）。③ **MAJOR（证据强度）**：`_acceptance_p2.py` P2-4 新增门禁项「Key 通路」+ 全量复跑 + CHANGELOG/本文档 limitation 补齐。④ **MINOR**：`SSEEvent.assess` 补 `verification` 字段（`vue-tsc` 切换 deferred）、简历 API 响应不再下发本地 `path`（`to_public_dict()`，内部 `to_dict()` 保留）、`delete_session` 改「先 `delete_thread` 后删行/Key」（失败可整体重试，贴合 N-8 三处同步）。验证：`pytest 280 passed`（+13）+ `ruff` 全通过 + `npm run build` 成功 + 全量验收 **门禁 passed=15 failed=0（退出码 0）／观察项 1 PASS／dep=0**（P2-4 Key 通路 `resolved_client_key='bocha-acceptance'`、`search_calls=3`）；修复前后 FAIL 证据与测试缺口（真实博查 Key 未验收、读回端 KV 契约偏隐式、本地历史库明文未清理、前端无类型检查）逐条落档 → CHANGELOG + §3/§5
...
| limitation·F1 | P2-2「题干含金标考点关键词」口径为**注入链路一致性**（fake LLM 原样回显考点清单块），不是真实「题目与清单相关性」证据；实测随抽取结果（LLM 非确定性）波动：89% → 91% → 80% → 84% → 82%（最终审查轮两轮复跑 38/45 与 37/45，标准 ≥80%，贴近阈值）→ 89%（2026-09-18 充值后复跑 40/45） | 真实相关性待后续用真实 LLM 抽样判定；建议同时固定抽样语料与缓解抖动 |
...
| 观察项·retry 路径 | `POST /api/resumes/{id}/retry`（R6「失败保留原文件可重试」）此前 pytest 与验收脚本均无覆盖；第 2 轮起由 `_acceptance_p2.py` 以**不计入门禁成功率**的观察项覆盖（对本次 failed 简历调用 retry → 断言 200 且状态 processing/ready + 原文件仍在磁盘；本次无 failed 简历时先构造 failed 探针覆盖该路径） | 已纳入验收脚本观察项（第 3 轮实测 PASS：探针 → http=200 / processing → ready 31.3s / 原文件保留=True；最终审查轮复跑 PASS：24.2s）；后续可提升为门禁项并补 pytest 用例 |
...
| **缺陷·搜索 Key 通路（BLOCKER）** | **已修复（`eb68045`，2026-09-18）**：原现象为配置页设置的博查 Key 永远不生效——`VerifyContext` 在 lifespan 构造期一次性固化 `BochaClient`（此时 KeyStore 无 Key）→ 客户端 `_api_key` 恒为空 → `search()` 恒抛 `BochaError` 被吞 → 恒返回 `unconfirmed / 搜索无可用信源`、`sources` 恒空。为何未被拦住：`tests/test_verify.py` 全注入 fake bocha（不校验 Key）；`_acceptance_p2.py` 用**类属性** patch `BochaClient.search` 恰好绕过 `_api_key` 检查。修复：`verify()` 惰性按当前 Key 解析客户端（KeyStore → `cfg.verify.api_key`，Key 变化才重建）+ `verify.enabled` 真正生效 + lifespan 种子 `.env` 的 `VERIFY_API_KEY`；测试侧新增「真实 `urlopen` 捕获 Authorization 头」用例与 `--only p2-4` Key 通路门禁项 | 已修复并验收：`resolved_client_key='bocha-acceptance'` + `search_calls=3`（修复前该门禁项 FAIL：`resolved_client_key=''`）；**遗留：真实博查 Key/出网从未验收**（仍为占位 Key + `search` 补丁） |
| **缺陷·API Key 明文落盘（MAJOR·安全）** | **已修复（`204d81c`，2026-09-18）**：P2-3 换 `SqliteSaver` 后 `_api_key` 沦为普通 state channel，本地 `data/interview.db` 的 checkpoint/writes 实测含 `_api_key` 与 `sk-` 明文（生产即用户真实 DeepSeek Key；`data/` 已 gitignore，未进版本库）。修复：**方案 A 持久化边界脱敏**——`SanitizingSqliteSaver.put_writes` 按通道名丢弃 `_api_key` 写入（该值为裸字符串 Key，仅靠 serde 递归删键抓不到）+ `put` 剔除 `updated_channels` 通道名 + `SanitizedSerde.dumps*` 递归剔键；`state.py` 注释与 `Score` 声明同步对齐 | 已修复：库文件 + `-wal/-shm` 原始字节断言（TDD 修复前失败）+ 重启续聊/报告路径无 KeyError；**遗留：本地历史库仍有修复前明文（3686 行 checkpoint 中 3436 行、writes 794 行），建议删库重建（本轮不代删）**；读回端依赖「调用方每次注入 + KeyStore 回退」约定，节点缺 Key 仍会 `KeyError`（无节点级防护） |
| deferred·前端类型检查 | 本轮仅给 `SSEEvent.assess` 补 `verification?: Verification \| null`；`web` 的 build 仍是 `vite build`，**未引入 `vue-tsc`**（切换工具链风险高，需先修既有类型错误） | 记为本轮 deferred，后续单独一轮处理 |
| limitation·测试缺口（最终审查轮） | ① 真实博查 Key/HTTP 出网、真实响应解析与限流仍无覆盖（P2-4/P2-5 均为占位 Key + 类属性补丁）；② 修复 1 的新用例是「假 `urlopen` + 真 `BochaClient`」，证明 Key 通路但不证明服务可用；③ 修复 2 的核心断言是「磁盘字节不含密钥」，未覆盖 `MemorySaver`（不落盘，无需）；④ `POST /finish` 重启后 KeyStore 回退 Key 无专项用例（由 chat 分支 + 验收 P2-5 端到端间接覆盖） | 如实落档，不淡化；后续补真实 Key 的可选联网验收（无 Key 时保持降级跳过） |
...
- 2026-09-18 **P2 最终审查修复轮（1 blocker + 2 major + 3 minor）**：① 搜索 Key 惰性解析（`VerifyContext` 不再构造期固化客户端，KeyStore → `cfg.verify.api_key`，Key 变化重建）+ `verify.enabled` 生效 + lifespan 种子 `.env` 的 `VERIFY_API_KEY`（`eb68045`）；② **API Key 不落盘**——选**方案 A 持久化边界脱敏**（`SanitizingSqliteSaver` 丢 `_api_key` 写入 + 剔 `updated_channels` 通道名；`SanitizedSerde` 递归剔键）而非方案 B（迁 `RunnableConfig`，需改全部节点与测试，回归面大），并修正 `state.py` 注释与 `Score` 声明（`204d81c`）；③ 验收新增「P2-4 Key 通路」门禁项 + 全量复跑 + limitation 补齐；④ minor：`SSEEvent.assess.verification`（`vue-tsc` deferred）、简历 API 不下载本地 `path`、`delete_session` 改「先 `delete_thread` 后删行/Key」（可整体重试）；`pytest 280` + `ruff` + `build` + 验收门禁 15/15 全通过；本地历史库明文建议删库重建已落档 → CHANGELOG + §3/§4/§5
- 2026-09-18 **P2 充值后真实 LLM 全量复跑确认**：DeepSeek 余额充值后全量复跑 `_acceptance_p2.py` → **门禁 passed=15 failed=0（退出码 0）／观察项 1 PASS／dep=0**——P2-1 解析成功率 20/20=100% + 字段命中率 20/20=100%、P2-2 考点清单 ready=20 与注入链路一致性 40/45=89%（题干命中 3/3）、P2-3 会话/对话历史恢复、P2-4 事实核验 verified + Key 通路（search_calls=3 全为占位 Key）+ 未配置 Key 对照、P2-5 端到端（第 1 题核验 verified、skip 无 error、报告 total_score=20 四维齐、导出 200）、retry 观察项 22.3s；本轮无 `app/` 代码改动，pytest 280 / ruff / build 维持 → CHANGELOG + §3/§5
