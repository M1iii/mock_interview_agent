...
**阶段：P2 交付完成 + 端到端验收通过（2026-09-17）——简历解析入库（P2-1）/ 双来源出题（P2-2）/ SqliteSaver 跨重启持久化（P2-3）/ web_verify 事实核验（P2-4）/ 端到端链路（P2-5）全部落地。验收脚本第 2 轮审查修复后复跑实测：门禁 passed=13 failed=1、观察项 1 项 PASS、依赖缺失 0（退出码 1）——**唯一门禁 FAIL 为脚本新增记录项首次暴露的 app 侧既有缺陷（流式 skip 出题，见 §5「缺陷·流式 skip 出题」）**，非本轮引入、待裁决；P1-3（28%）与 P1-4 decline 子项搁置项顺延，待真实检索环境恢复后复测**
...
| 观察项·retry 路径 | `POST /api/resumes/{id}/retry`（R6「失败保留原文件可重试」）此前 pytest 与验收脚本均无覆盖；第 2 轮起由 `_acceptance_p2.py` 以**不计入门禁成功率**的观察项覆盖（对本次 failed 简历调用 retry → 断言 200 且状态 processing/ready + 原文件仍在磁盘；本次无 failed 简历时先构造 failed 探针覆盖该路径） | 已纳入验收脚本观察项（第 2 轮实测 PASS：探针 → http=200 / processing → ready 30.5s）；后续可提升为门禁项并补 pytest 用例 |
| **缺陷·流式 skip 出题** | **跳过非末题后下一题出题失败**（SSE `error{"message": "服务异常，请重试"}`，题号不推进）。根因：`app/api/chat.py` skip 分支传**裸 `handler`**（非 `[handler]`）→ `app/llm/client.py:100` `config={"callbacks": handler}` → `langchain ensure_config` 对 `COPIABLE_KEYS`（含 callbacks）执行 `v.copy()` → `TokenStreamHandler` 无 `copy` → `AttributeError` 被包装为 `LLMError`。由第 2 轮验收脚本新增的 skip error 记录项首次暴露（**非本轮引入**，两轮运行 100% 复现；此前因 skip 循环静默 break + `POST /finish` 兜底出报告而被掩盖） | 待用户裁决后单独修复（建议 `chat.py` 改传 `[handler]` 或补 `TokenStreamHandler.copy()`，并加走真实 `complete_sync` 的回归测试；本轮禁改 `app/`） |
...
- 2026-09-17 P2 验收审查修复（第 2 轮）：验收脚本健壮性——依赖缺失（无 ready 语料）归类 `dep` 不计入 failed、skip 循环 error 事件显式记录（门禁项）、5 处 `assert` 改「记录 FAIL + 提前 return」+ main 每项兜底 try/except（保证总有 SUMMARY 与退出码）、摘要 detail 打印原始 `summary`（截断）；新增 retry 路径观察项（`kind="obs"`，不计入成功率门禁，无 failed 简历时构造探针）；CHANGELOG / 本文档「随本提交」表述补 `20216a5` 消歧 → CHANGELOG + §3/§4/§5。**副产物：minor-2 记录项首次暴露 app 侧既有缺陷「流式 skip 出题失败」**（`chat.py` 传裸 handler → `TokenStreamHandler` 无 `copy` → `AttributeError` → SSE error；非本轮引入、两轮 100% 复现，此前被 skip 静默 break + `POST /finish` 兜底掩盖），本轮禁改 `app/` 故仅落档 §5 待裁决
